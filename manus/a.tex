...
Now, examine the interface between $g^{\star}$ and $n_i$. Due to grain growth, recrystallization, or other processes, this moves with velocity $v_i$ outwardly normal to the surface. Thus, taken over all neighboring grains, $\frac{dV^{\star}}{dt}=\sum_{i}s_i v_i$.

Now, suppose we have a polycrystal with a large number of grains, and the neighbors of $g^{\star}$ are random. Then,
$E[\frac{dV^{\star}{dt}=n E[s_i v_i]=n*

Furthermore, suppose that $q$ is a function of the intensive properties $p_1, p_2,...$ of the grains $g_1$ and $g_2$ on either side of the boundary, 

$g$ be an arbitrary grain, which has neighbors $n_i$ each accounting for a surface area $s_i$ shared with $g$, such that $S=\sum n_i$.
